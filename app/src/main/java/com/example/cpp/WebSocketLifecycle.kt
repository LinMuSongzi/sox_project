...
class WebSocketLifecycle private constructor(val savedStateHandle: ISaveStateHandle) :
    ...
    private var webSocketClient: MyWebSocket? = null
    ...
    internal class MyWebSocket(var uri: URI, val savedStateHandle: ISaveStateHandle) :
        WebSocketClient(uri) {
    ...
    override fun connect(uri: String) {
        if (webSocketClient == null) {
            webSocketClient = MyWebSocket(URI.create("ws://$uri"), savedStateHandle)
        }
        if (webSocketClient?.isOpen == true) {
            webSocketClient?.closeBlocking()
    ...
    override fun connect() {
        if (webSocketClient == null) {
            webSocketClient = MyWebSocket(URI_STRING, savedStateHandle)
        }
        if (webSocketClient?.isOpen != true) {
            try {
                Log.i(TAG, "connect: uri = "+webSocketClient?.uri)
